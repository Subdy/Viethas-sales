...
@Component({
  ...
  templateUrl: "./expenditure-add.page.html",
  styleUrls: ["./expenditure-add.page.scss"]
})
export class ExpenditureAddPage implements OnInit {
  private user_info;
  validations_form: FormGroup;
  errorMessage: string = "";
  private triggerPopupValidation: boolean = false;
  private triggerPopupValidation_message: string =
    "Bạn phải nhập đúng các trường cần thiết";
  validation_messages = {
  ...
  onSubmit(form) {
    ...
    let data = form.value;
    data["bill_code"] = this.genNewBillCode(); // Add bill_code
    ...
    this.firebaseQuery
      .createTask(this.collection, data)
      .then(
        res => {
        ...
        err => {
          this.triggerPopupValidation_message =
    ...
    this.firebaseQuery
      ...
      .then(
        res => {
          for (let i = 0; i < res.docs.length; i++) {
            this.firebaseQuery
              .deleteTask(this.collection, res.docs[i].id)
              .then(
                res => {},
                err => {}
              )
